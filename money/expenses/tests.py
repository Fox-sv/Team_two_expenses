...
import datetime
from . import views, courses_money


class EntryTest(TestCase):
    def setUp(self):
        self.category_1 = Category.objects.create(name='salary')
        self.category_2 = Category.objects.create(name='food')
        ...
        #                                     type_inc_exp='income')
        for i in range(1,10):
            Entry.objects.create(summa=150*i, notes='salary_{}'.format(i), category=self.category_1, currency='BYN',
                                 responsible_user_id=self.responsible_user_id, date_money='2020-11-18',
                                 type_inc_exp='income')

        for i in range(1,10):
            Entry.objects.create(summa=97*i/3, notes='salary_{}'.format(i), category=self.category_1, currency='EU',
                                 responsible_user_id=self.responsible_user_id, date_money='2020-11-18',
                                 type_inc_exp='income')
        for i in range(1,90):
            Entry.objects.create(summa=97*i/3, notes='salary_{}'.format(i), category=self.category_1, currency='EU',
        ...
        for i in range(20,40):
            Entry.objects.create(summa=36*i/7, notes='food_{}'.format(i), category=self.category_1, currency='BYN',
                                 responsible_user_id=self.responsible_user_id, date_money='2020-11-18',
    ...
    def test_total_balance(self):
        print(views.calculate_balance(['BYN']))
